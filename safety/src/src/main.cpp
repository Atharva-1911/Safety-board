...
// functions coming from other cpp files
bool setupOTA();
...
volatile bool otaInProgress = false;  // Flag to indicate if OTA update is in progress
...
void loop() {
    ...
    // Check for OTA updates and stop working
    ArduinoOTA.handle();
   ...
   bool estop = check_estop_state();
    ...
    taskENTER_CRITICAL(&limit_state_spinlock);
        ...
        if (limit_stable_state & (1 << 3)) {
        relay_state |= (~J3_DRIVER_RELAY & 0xFFFF);
        }

        if (limit_stable_state & (1 << 4)) {
        relay_state |= (~J4_DRIVER_RELAY & 0xFFFF);
        }

        if (limit_stable_state & (1 << 5)) {
        relay_state |= (~J5_DRIVER_RELAY & 0xFFFF);
